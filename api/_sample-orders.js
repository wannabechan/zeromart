...
/** 2026-01-15부터 오늘(KST)까지의 매주 월요일·화요일 (1/15는 목요일 → 첫 월 1/19, 첫 화 1/20) */
...
/**
 * @param {Array} stores - getStores() 결과 (어드민 등록 순서 = 1번/2번/3번 매장)
...
function getSampleOrders(stores, menusByStore) {
  ...
  const orders = [];
  const { mondays, tuesdays } = getSampleOrderDates();
